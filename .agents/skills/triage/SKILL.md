...
These are canonical role names. Read the actual label mapping from
`docs/agents/triage-labels.md`. If it is missing, stop and report that required repository
...
4. **Grill (if needed).** If the request needs fleshing out, call the Skill tool twice, for
   "grilling" and "domain-modeling", and grill it into shape a round of questions at a time,
   sharpening domain terms and updating the repository's existing intent and decision records as
   decisions land. Follow `docs/agents/domain.md` and `docs/agents/skill-governance.md`.
